            ...
            Using cnn As New SqlClient.SqlConnection(SimpleDataApp.Utility.GetConnectionString())
                ...
                Using cmd As New SqlClient.SqlCommand(sql, cnn)
                    ...
                    For Each row As DataGridViewRow In Me.DataGridView1.Rows
                        ...
                        End If
                        'date   caduca
                        dateCaduca = Date.Now.Date
                        'apartirde
                        Cant = row.Cells("ClnCantidad").Value
                        'caduciada
                        isCaduca = row.Cells("ClnFechaExp").Value

                        percentageDescuent = DecimalFromString(row.Cells("ClnPorcenta").Value) / 100
                        'date caduca
                        If isCaduca Then
                            dateCaduca = row.Cells("clnFecha").Value
                        End If
                        ...
                        cmd.Parameters("@Caducidad").Value = isCaduca
