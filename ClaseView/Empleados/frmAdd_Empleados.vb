...
Public Class frmAdd_Empleados
                    ...
                    Using adat As New SqlDataAdapter(cmd)
                        Using dt As New DataTable
                            ...
                            If dt.Rows.Count > 0 Then
                                ...
                                If IsNothing(dt(0)("reporta_A")) Then
                                    If Integer.Parse(dt(0)("reporta_A").ToString) > 0 Then
    ...
    Private Function Update_(idempleado As Integer) As Boolean
        Try
            Using cmd As New EmpleadosTableAdapter
                ...
                End If
                If cmd.UpdateEmpledo(titulo, cargo, sueldoTextBox.Value,
                                    dateEnter, idempleado) = 1 Then
                    Return True
                Else
                    Return False
                End If
        ...
        Try
            Using cmd As New EmpleadosTableAdapter
                ...
                If dtFecha_Ingreso.Checked Then
                    dateEnter = dtFecha_Ingreso.Value
                End If
                If cmd.InsertEmployee(Me.idPersona, titulo, cargo, sueldoTextBox.Value,
                                    dateEnter, Me.idEmpleado) = 1 Then
                    Return True
                Else
                    Return False
                End If
    ...
    Private Function ValidaData() As Boolean
        If CargoBombox.SelectedIndex = -1 Then
    ...
    Private Sub btnRepot_Click(sender As Object, e As EventArgs) Handles btnRepot.Click
        Try
            Using newlistEmploye As New frmList_Empleados(stateClient.Cliente)
                With newlistEmploye
                    .Text = "Seleccione a quien debe reportarse"
                    .ShowDialog()
                    If .DialogResult = DialogResult.OK Then
                        idReportTo = .idEmpleado
                        sql = .dtg.SelectedCells(.dtg.Columns("Nombres").Index).Value
                        sql = sql & " (" & .dtg.SelectedCells(.dtg.Columns("Ruc_Ci").Index).Value
                        sql = sql & ")"
                        Me.txtNameReportto.Text = sql
                    End If
                End With
            End Using
