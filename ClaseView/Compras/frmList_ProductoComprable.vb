...
Public Class frmList_ProductoComprable
    ...
    Private Sub frmList_ProductoComprable_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        End If
    End Sub
    Sub Carga_listProductAll(LikeNom_Comercial As String, ByVal isBarCod As Boolean)
        ...
        Try
            ...
            If (isBarCod) Then
                dt = tap.GetDataByBarCode(LikeNom_Comercial)
            Else
            ...
            dtg.Columns(dtg.Columns("codProducto").Index).HeaderText = "Código producto"
            ...
            AddHandler childView.newGrid.RowEnter, AddressOf dtg_RowEnterChild

            dtg_RowEnter(dtg, New DataGridViewCellEventArgs(0, 0))

        Catch ex As Exception
            MsgBox(ex.Message & vbLf & ex.StackTrace, vbCritical, "Error")
        Finally
            estaCargado = True
    ...
    Private Sub dtg_RowEnterChild(sender As Object, e As DataGridViewCellEventArgs)
        If (estaCargado) Then
    ...
    Private Sub dtg_RowStateChanged(sender As Object, e As DataGridViewRowStateChangedEventArgs)
        ...
        If e.Row.Selected AndAlso estaCargado AndAlso e.StateChanged = DataGridViewElementStates.Selected Then
            If sender.SelectedRows.Count = 1 Then
                idProducto = sender.SelectedCells.item(sender.columns("idProducto").index).Value
                idPresent_Master = sender.SelectedCells.item(sender.columns("idPresentacion").index).Value
                Nom_Comercial = sender.SelectedCells.item(sender.columns("Nom_comercial").index).Value
    ...
    Private Sub FindButton_Click(sender As Object, e As EventArgs) Handles FindButton.Click
        If FindTextBox.TextLength > 2 Then
            ...
            Dim mySpliter As ResponseSpliter = GenerateSpliter(FindTextBox.Text)
            sql = String.Empty
            If mySpliter.IsSucces Then
                ...
                Carga_listProductAll(If(mySpliter.IsNumeric, mySpliter.Spliter(0),
                                     "%" & sql & "%"), mySpliter.IsNumeric)
            End If
        ...
        Try
            ...
            With MDI_AddProdcutos
                ...
                If .DialogResult = DialogResult.OK Then
                    sql = .Nom_Comerial
                    Carga_listProductAll(.Nom_Comerial, False)
                End If
            End With
    ...
    Private Sub EditButton_Click(sender As Object, e As EventArgs) Handles EditButton.Click
        Try
            If (dtg.SelectedRows.Count = 1) Then
                Me.Cursor = Cursors.WaitCursor
                Me.idProducto = CType(dtg.SelectedCells.Item(dtg.Columns("idProducto").Index).Value, Integer)
                Using MDI_AddProdcutos
                    With MDI_AddProdcutos
                        .flag = "Modificar"
                        .id_Producto = Me.idProducto
                        .ShowDialog()
                        If (.DialogResult = DialogResult.OK) Then
                            sql = .Nom_Comerial
                            Carga_listProductAll(sql, False)
                        End If
                    End With
                End Using
            End If
